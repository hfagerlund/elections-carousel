...
//var jQuery = require('jquery'); //TEST_CODE
...
var ElectionsCarousel = (function($) {
  ...
  var _setBarGraphHeights = function (options) {
    ...
    mainStyle.insertRule(_config.css.ridingIdRoot + options.id + ' '+ _config.css.listCandidateRoot + options.party + '{height:' + barHeight +'px!important;position:relative}', oLength);
  };
  ...
  var _calculateCandidateVotePercentages = function (data){
    ...
    for(var i = 0; i < data.length; i++) {
      ...
      riding.results.forEach(function(candidate){
        ...
        var candidateVotePercentage = _roundToTwoDec((candidateVotes/totalRidingResultsVotes)*100); 
        candidate.votePercentage = candidateVotePercentage;
	_setBarGraphHeights({ id         : ridingId,
  ...
  var _sortCandidatesByDescendingVotes = function (data){
    for(var i = 0; i < data.length; i++) {
      ...
      var ridingResults = riding.results;
      //descending order
      var ridingResultsByVotes = ridingResults.sort(function(a, b){return b.votes-a.votes;});
      $(this).parent.resultsByVotes = ridingResultsByVotes;
    }
  };

  var _rotateSlides = function(newWrapperPosition, transitionTime){
    $(_config.css.ridingWrapper).animate({left: newWrapperPosition}, transitionTime, function() { /* action on animation completion */ });
  };
  ...
  var _calculateHorizontalPosition = function (whichControl, data){
    ...
    var newWrapperPosition = _initialize.initWrapperPosition - (_initialize.movementCounter*slideWidth);
    ...
    if(newWrapperPosition >= _initialize.maxRightHorizontalPosition){
      ...
      if(newWrapperPosition > _initialize.maxRightHorizontalPosition){ //reached 1st slide/max. Back limit
	...
	newWrapperPosition = _initialize.maxRightHorizontalPosition;
      ...
      if(newWrapperPosition < maxLeftHorizontalPosition){ //reached last slide - ie. max Next limit
        //stop movement and reset for next click
        _initialize.movementCounter = (numberOfSlides - 1);
	newWrapperPosition = maxLeftHorizontalPosition;
      }
    } else { //within allowable range of movement
      _rotateSlides(newWrapperPosition, data.transitionTime);
      _reEnableCtrl(_config.css.controlNext);
      _reEnableCtrl(_config.css.controlBack);
    }
  ...
  var _bindCtrlListener = function (events, data){
    $( document ).on( events, _config.css.control, function(e) { //jQuery 1.7+
      if(e.type === 'click'){ //Enter key, spacebar, or mouse click
  ...
  var _renderTemplate = function (data){
    ...
    $.get(data.templatePath, function(templates) {
      var template = $(templates).filter(data.templateId).html();
  ...
  var extendData = function(data, headingTxt, slidewidth, templateid, templatepath, transitiontime){
...
//      return data.pollsReported; //TEST_CODE
      _renderTemplate(data);
  };

  var _getJsonData = function(url, callback, headingtext, slidewidth, templateid, templatepath, transitiontime){
    $.ajax({
      type: 'GET',
      url: url,
  ...
  var main = function(options){
    //set default values
    options.url = typeof options.url !== 'undefined' ? options.url : 'http://127.0.0.1:8080/src/assets/fixtures/results.js'; 
    options.callback       = typeof options.callback       !== 'undefined' ? options.callback       : 'gNews_getRidingDetailsCallback';
    options.headingtext    = typeof options.headingtext    !== 'undefined' ? options.headingtext    : '2011 Election Results';
    options.slidewidth     = typeof options.slidewidth     !== 'undefined' ? options.slidewidth     : 600;
    options.templateid     = typeof options.templateid     !== 'undefined' ? options.templateid     : '#viewTemplate';
    options.templatepath   = typeof options.templatepath   !== 'undefined' ? options.templatepath   : 'http://127.0.0.1:8080/src/assets/templates/viewFull.mustache';
    options.transitiontime = typeof options.transitiontime !== 'undefined' ? options.transitiontime : 300;  

    _getJsonData(options.url, options.callback, options.headingtext, options.slidewidth, options.templateid, options.templatepath, options.transitiontime);
  };


  return {
    init: main,
//    extendData: extendData //TEST_CODE
  };
